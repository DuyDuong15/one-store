...
const SearchPage = () => {
    ...
    const filteredProducts = useMemo(() => {
        return products.filter((product) => {
            ...
            const matchesCategory = selectedCategories.length === 0 || (product.attributeSetIdentifier !== null && selectedCategories.includes(product.attributeSetIdentifier));
        ...
        <div className='min-h-screen bg-gray-900 text-gray-100 p-8'>
            <div className='max-w-7xl mx-auto'>
                ...
                <div className='flex flex-col lg:flex-row gap-8'>
                                            ...
                                            {categories.map((category) => (
                                                <div key={category} className='flex items-center'>
                                                    <Checkbox
                                                        ...
                                                        checked={category !== null && selectedCategories.includes(category)}
                                                        onCheckedChange={(checked) => {
                                                            setSelectedCategories(
                                                                checked ? [...selectedCategories, category!].filter(Boolean) : selectedCategories.filter((c) => c !== category)
                                                            );
                                                        }} />
                                                    <Label htmlFor={category ?? undefined} className='ml-2 text-sm text-gray-300'>
                                                        {category}
                                                    </Label>
                                                </div>
                                            ))
                    ...
                    <LayoutGroup>
                        <motion.div className={`w-full ${showFilters ? "lg:w-3/4" : ""}`}
                            layout
                            ...
                            <AnimatePresence mode='wait'>
                                {
                                    ...
                                    >
                                        ...
                                        <motion.div key="products"
                                            ...
                                            animate={{ opacity: 1 }}
                                            exit={{ opacity: 0 }}
                                            className={`grid grid-cols-1 md:grid-cols-3 lg:grid-cols-${showFilters ? "3" : "4"} gap-6`}
                                        >
                                            <AnimatePresence>
                                                {
